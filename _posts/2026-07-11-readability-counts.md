...
completion and other tools that save keystrokes, so why have some obscure *i - ...* when it could 
be *found_items = ...*

Another very important idea here is introspection, where defined objects themselves can be examined 
for information.  When defining a new class of function, it is not unusual to add some useful 
comment, but put in tripple quotes and it then becomes help available from the command line or the 
editor.  So easy and so useful.
